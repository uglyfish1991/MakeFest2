...
def search_a(timeb): #takes variable from function call in main - couldn't do this any other way without circular imports?
    ...
    global seen, seen_before, answer_expected, ans1
    if not seen_before or not answer_expected:
...
# shows 2 searches for the number given - one is previous users, one is ours
    ...
    {bcolors.OKCYAN}<aa0ExSEARCH_SYS_LOGS()> "{ans1}" at {our_search} {bcolors.ENDC}
...
# shows 1 result, the time the hacker changed the password
        ...
        if not answer_expected:
            fast_text(f"{bcolors.FAIL}0 matching item(s) found in {search_place}{bcolors.ENDC}\n")      
...
# shows 4 results - users with that DOB
        ...
        fast_text(f"Searching for {ans1} in the {search_place} \n")
        wait_text(f"{bcolors.OKBLUE}. . .{bcolors.ENDC}\n")
        if not answer_expected and ans1.isdigit():
            fast_text(f"{bcolors.OKCYAN}2 matching item(s) found in {search_place}{bcolors.ENDC}\n")

            print(f"""
            {bcolors.WARNING}Name:{bcolors.ENDC} Griffiths, Mark    {bcolors.WARNING}D.O.B:{bcolors.ENDC} {ans1}    {bcolors.WARNING}User Number:{bcolors.ENDC} LCC073447     {bcolors.WARNING}Current:{bcolors.ENDC} 0 pending    {bcolors.WARNING}Notes:{bcolors.ENDC} N/A
            {bcolors.WARNING}Name:{bcolors.ENDC} Marshall, Emma     {bcolors.WARNING}D.O.B:{bcolors.ENDC} {ans1}    {bcolors.WARNING}User Number:{bcolors.ENDC} LCC061091     {bcolors.WARNING}Current:{bcolors.ENDC} 2 pending    {bcolors.WARNING}Notes:{bcolors.ENDC} Waiting List GAME_Asset<34873>

    """)

        elif not answer_expected and not ans1.isdigit():
            fast_text(f"{bcolors.FAIL}0 matching item(s) found in {search_place}{bcolors.ENDC}\n")     
            ...
            {bcolors.WARNING}Name:{bcolors.ENDC} Gregor, James     {bcolors.WARNING}D.O.B:{bcolors.ENDC} 09/04/77     {bcolors.WARNING}User Number:{bcolors.ENDC} LCC0998947     {bcolors.WARNING}Current:{bcolors.ENDC} Banned       {bcolors.WARNING}Notes:{bcolors.ENDC} {bcolors.FAIL}12 Unreturned - MAX FINE{bcolors.ENDC}
    ...
    """)
        ...
        if not answer_expected and ans1.isdigit():
            ...
            fast_text(f"{bcolors.OKCYAN}309 matching item(s) found in {search_place}{bcolors.ENDC}\n")

            print(f"""
            {bcolors.WARNING}Title:{bcolors.ENDC} A Study of Marine Life    {bcolors.WARNING}Author:{bcolors.ENDC} Craig Inkson     {bcolors.WARNING}ISBN:{bcolors.ENDC} 48392{ans1}   {bcolors.WARNING}Genre:{bcolors.ENDC} Science/Biology/Marine
            {bcolors.WARNING}Title:{bcolors.ENDC} Mountains of Asia         {bcolors.WARNING}Author:{bcolors.ENDC} Shinske Takyama  {bcolors.WARNING}ISBN:{bcolors.ENDC} 00907{ans1}   {bcolors.WARNING}Genre:{bcolors.ENDC} Geography/Asia/Geology
            {bcolors.WARNING}Title:{bcolors.ENDC} Arabic for Beginners      {bcolors.WARNING}Author:{bcolors.ENDC} Ayesha Izmol     {bcolors.WARNING}ISBN:{bcolors.ENDC} 00678{ans1}   {bcolors.WARNING}Genre:{bcolors.ENDC} Language/Arabic
...
# search_a(4) remember to comment this out when adding to main.py